...
3. If development mode is off, the plugin opens `https://app.lex-able.com` for OAuth PKCE. The user signs in on Lexable. Cursor receives an access token and server-side entitlements. Do not invent a successful login if discovery or token exchange fails.
